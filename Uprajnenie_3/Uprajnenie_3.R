...
new.df <- rbind(new.df, cbind(df[df$Reporter %in% CIS, ], data.frame(Group = 'CIS')))
new.df <- rbind(new.df, cbind(df[df$Reporter %in% customs_union, ], data.frame(Group = 'Custom Union')))
new.df <- rbind(new.df, cbind(df[!(df$Reporter %in% CIS) & !(df$Reporter %in% customs_union), ],
                                                data.frame(Group = 'Other countries')))
