...
write.csv(df, file = './data/WDI.csv', row.names = F)
## ------------------------------------------------------------
...
dataset_id <- '8911021440-obrazovanie'
...
doc <- doc[grep('поселок Пурпе-1', doc$Address, invert = T), ]
...
write.csv2(doc, file = './data/yandex_maps.csv', row.names = F)
